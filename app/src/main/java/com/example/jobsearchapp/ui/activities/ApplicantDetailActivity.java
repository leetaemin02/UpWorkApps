...
public class ApplicantDetailActivity extends BaseActivity {
    ...
    private View btnAccept, btnReject, cvAiEvaluation;
    private TextView tvProcessedStatus, tvAiDetailScore, tvAiDetailReason, tvAiEvaluationLabel;
    ...
    protected void initViews() {
        ...
        cvAiEvaluation = findViewById(R.id.cvAiEvaluation);
        tvAiDetailScore = findViewById(R.id.tvAiDetailScore);
        tvAiDetailReason = findViewById(R.id.tvAiDetailReason);
        tvAiEvaluationLabel = findViewById(R.id.tvAiEvaluationLabel);
        ...
        if (applicant != null) {
            displayBasicInfo();
            loadFullProfile();
            fetchAiEvaluation(); // Re-fetch live AI score from Firestore
        } else {
    ...
    private void displayBasicInfo() {
        ...
        if (applicant.getAiScore() != null && applicant.getAiScore() > 0) {
            // Show cached score immediately while live fetch is in progress
            if (cvAiEvaluation != null) cvAiEvaluation.setVisibility(View.VISIBLE);
            if (tvAiEvaluationLabel != null) tvAiEvaluationLabel.setVisibility(View.VISIBLE);
            if (tvAiDetailScore != null) tvAiDetailScore.setText(applicant.getAiScore() + "%");
            if (tvAiDetailReason != null) tvAiDetailReason.setText("AI nhận xét: " + (applicant.getAiReason() != null ? applicant.getAiReason() : "Chưa có nhận xét chi tiết."));
        } else {
            // Show placeholder — live fetch will update this
            if (cvAiEvaluation != null) cvAiEvaluation.setVisibility(View.VISIBLE);
            if (tvAiEvaluationLabel != null) tvAiEvaluationLabel.setVisibility(View.VISIBLE);
            if (tvAiDetailScore != null) tvAiDetailScore.setText("...");
            if (tvAiDetailReason != null) tvAiDetailReason.setText("Đang chờ AI đánh giá CV...");
        }
    ...
    /**
     * Re-fetch aiScore and aiReason directly from Firestore.
     * The Cloud Function writes these fields asynchronously after the application is created,
     * so the serialized Applicant object passed via Intent may not have them yet.
     */
    private void fetchAiEvaluation() {
        String appId = applicant.getApplicationId();
        if (appId == null || appId.isEmpty()) return;

        db.collection("applications").document(appId).get()
            .addOnSuccessListener(doc -> {
                if (!doc.exists()) return;

                Long scoreRaw = doc.getLong("aiScore");
                String reason = doc.getString("aiReason");

                if (scoreRaw != null) {
                    int score = scoreRaw.intValue();
                    if (cvAiEvaluation != null) cvAiEvaluation.setVisibility(View.VISIBLE);
                    if (tvAiEvaluationLabel != null) tvAiEvaluationLabel.setVisibility(View.VISIBLE);
                    if (tvAiDetailScore != null) tvAiDetailScore.setText(score + "%");
                    if (tvAiDetailReason != null) {
                        String displayReason = (reason != null && !reason.isEmpty())
                                ? "AI nhận xét: " + reason
                                : (score == 0 ? "AI không đọc được nội dung CV (có thể là PDF scan/ảnh)." : "Chưa có nhận xét chi tiết.");
                        tvAiDetailReason.setText(displayReason);
                    }
                } else {
                    // Cloud Function chưa chạy xong hoặc không có CV
                    if (tvAiDetailScore != null) tvAiDetailScore.setText("N/A");
                    if (tvAiDetailReason != null) {
                        String cvUrl = doc.getString("cvUrl");
                        if (cvUrl == null || cvUrl.isEmpty()) {
                            tvAiDetailReason.setText("Ứng viên chưa đính kèm CV — AI không thể đánh giá.");
                        } else {
                            tvAiDetailReason.setText("AI đang xử lý CV, vui lòng thử lại sau ít phút.");
                        }
                    }
                }
            })
            .addOnFailureListener(e -> {
                android.util.Log.w("ApplicantDetail", "Không tải được AI evaluation: " + e.getMessage());
                if (tvAiDetailReason != null) tvAiDetailReason.setText("Không thể tải đánh giá AI.");
            });
    }
